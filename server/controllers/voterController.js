...
const registerVoter = async (req, res, next) => {
    try {
        ...
        // No user/voter should be admin except for one with email "waliza.cse.20230104029@aust.edu"
        let isAdmin = false;
        if(newEmail == "waliza.cse.20230104029@aust.edu")
        {
            isAdmin = true
        }
        //save new voter to db
        const newVoter = await VoterModel.create({fullName, email: newEmail, password: hashedPassword,
             isAdmin})
